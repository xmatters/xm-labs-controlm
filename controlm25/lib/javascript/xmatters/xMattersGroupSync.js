      ...
      else if (this.dataSyncHelper.checkIncludeExcludeList("Group", group.name))
      {
        if (group.exists())
        {
          if (!GROUP_SEED_ONLY)
          {
            action = "Update Group: ";
            name = group.name;
            this.log.debug("Sending group update for " + group.name);
        ...
        else 
        {
          if (!group.updateOnly)
          {
            ...
            name = group.team.teamName;
            group.team.add();
            this.log.debug("Sent team add for " + group.team.teamName);
              
            action = "Add Coverage: ";
            name = group.coverage.name;
            group.coverage.add();
            this.log.debug("Sent coverage add for " + group.coverage.name);
          }
        }
      } 
    }
